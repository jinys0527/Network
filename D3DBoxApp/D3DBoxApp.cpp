...
struct Object
{
	...
	{
		m_Pos = m_DestPos = start;
		m_Moving = false;
        World = Matrix::CreateScale(cellSize, 1.0f, cellSize) * Matrix::CreateTranslation(m_Pos);
        Dest = World;
        Start = World;
	}

	void Move(const Vector3& dest, float cellSize)
	{
		m_DestPos = dest;
		m_Moving = false;
        Start = World;
        Dest = Matrix::CreateScale(cellSize, 1.0f, cellSize) * Matrix::CreateTranslation(m_DestPos);
	}
...
struct Grid
{
    ...
    {
        return x < 2 * m_HalfCells && x >= 0 && z >= 0 && z < 2 * m_HalfCells;
    }
};
...
struct App
{
    ...
    ComPtr<ID3D11ShaderResourceView> m_GreenTexSRV;
    ...
    // Transform
    Object                           m_Box;
    std::unordered_map<int, Object>  m_Boxes;
    int m_MyId = -1;    //블록 ID -> 서버가 SPAWN으로 부여
    ...
    ~App()
    {
        m_Client->Send("LEAVE " + std::to_string(m_MyId) + "\n");
    ...
    void CreateGridVB()
    {
        ...
        Vector3 cMajor(1.0f, 1.0f, 1.0f); // 노란색
    ...
    void UpdateAndDraw()
    {
		...
		stride = sizeof(VertexPTN); offset = 0;
		...
		if (!m_Boxes.empty())
		{
			...
			{
				MapAndSetCB(box.World, m_Camera.m_View * m_Camera.m_Proj);
        ...
        {
            MapAndSetCB(greenBox.World, m_Camera.m_View * m_Camera.m_Proj);
...
//             m_Grid.SetType(x, z, Type::Box);
			...
			{
				SendMoveRequestToServer(c);
			}
			else
			{
				SendSpawnRequestToServer(c);
			}
        }
    }

    void OnMClick(int mx, int my)
    {
        Vector3 ro, rd; ScreenRay(mx, my, ro, rd);
        Vector3 hit;
        if (RayHitGround(ro, rd, hit) && isAnimEnd)
    ...
    void ProcessNetwork()
    {
        ...
        while (m_Client->PopLine(line))
        {
			...
			else if (cmd == "SPAWN")
			{
				...
				Vector3 pos(
					(x + 0.5f) * m_Grid.m_CellSize,
					0.0f,
					(z + 0.5f) * m_Grid.m_CellSize
				);

				Object box;
				box.Init(pos, m_Grid.m_CellSize);
			...
			else if (cmd == "MOVE")
			{
				...
				// 1단계: 즉시 위치 이동 (텔레포트)
				it->second.Move(pos, m_Grid.m_CellSize);
			}
	...
	{
		int cellX = static_cast<int>(cellCenter.x - 0.5f / m_Grid.m_CellSize);
		int cellZ = static_cast<int>(cellCenter.z - 0.5f / m_Grid.m_CellSize);
